...
GateThresholdNumber = HMMD_WAVE_ns.class_("GateThresholdNumber", number.Number)
LightThresholdNumber = HMMD_WAVE_ns.class_("LightThresholdNumber", number.Number)
MaxDistanceTimeoutNumber = HMMD_WAVE_ns.class_("MaxDistanceTimeoutNumber", number.Number)
...
async def to_code(config):
    ...
    if timeout_config := config.get(CONF_TIMEOUT):
        ...
        await cg.register_parented(n, config[CONF_HMMD_WAVE_ID])
        cg.add(HMMD_WAVE_component.set_timeout_number(n))
    if max_move_distance_gate_config := config.get(CONF_MAX_MOVE_DISTANCE_GATE):
        ...
        await cg.register_parented(n, config[CONF_HMMD_WAVE_ID])
        cg.add(HMMD_WAVE_component.set_max_move_distance_gate_number(n))
    if max_still_distance_gate_config := config.get(CONF_MAX_STILL_DISTANCE_GATE):
        ...
        await cg.register_parented(n, config[CONF_HMMD_WAVE_ID])
        cg.add(HMMD_WAVE_component.set_max_still_distance_gate_number(n))
    if light_threshold_config := config.get(CONF_LIGHT_THRESHOLD):
        n = await number.new_number(
            light_threshold_config, min_value=0, max_value=255, step=1
        )
        await cg.register_parented(n, config[CONF_HMMD_WAVE_ID])
        cg.add(HMMD_WAVE_component.set_light_threshold_number(n))
    for x in range(9):
        if gate_conf := config.get(f"g{x}"):
            ...
            await cg.register_parented(n, config[CONF_HMMD_WAVE_ID])
            cg.add(HMMD_WAVE_component.set_gate_move_threshold_number(x, n))
            ...
            await cg.register_parented(n, config[CONF_HMMD_WAVE_ID])
            cg.add(HMMD_WAVE_component.set_gate_still_threshold_number(x, n))
